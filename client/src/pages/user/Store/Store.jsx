        ...
        <div className="relative bg-gradient-to-br from-[#d9e8f5] via-[#e2ebf4] to-[#f4f7fa] aspect-square flex items-center justify-center overflow-hidden p-4">
          <motion.img
            ...
            className={`absolute inset-0 h-full w-full object-contain mix-blend-multiply transition-all duration-500 ${
              showAltImage ? "scale-[1.03] opacity-0 blur-[2px]" : "scale-100 opacity-100 blur-0"
            }`}
          />
          ...
          {secondaryImage && (
            <motion.img
              ...
              className={`absolute inset-0 h-full w-full object-contain mix-blend-multiply transition-all duration-500 ${
                showAltImage ? "scale-[1.05] opacity-100 blur-0" : "scale-[1.01] opacity-0 blur-[6px]"
              }`}
            />
          )}
          ...
          <div className="absolute top-3 left-3">
            <span className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-white/60 backdrop-blur-md border border-white/80 text-[10px] font-bold text-gray-700 shadow-sm">
              <span className={`w-1.5 h-1.5 rounded-full ${stockStatus.dot} shrink-0`} />
              {stockStatus.label}
            </span>
          </div>
...
const StorePage = () => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeCategory !== "all") {
      list = list.filter((product) => {
        return getCategoryKey(product.category) === activeCategory;
      });
    }
  ...
  const categories = useMemo(() => {
    ...
    allProducts.forEach((product) => {
      const id = getCategoryKey(product.category);
  ...
  return (
    <div
      ...
      style={{
        fontFamily: "'SF Pro Display', 'DM Sans', -apple-system, BlinkMacSystemFont, sans-serif",
      }}
    >
            ...
            >
              ...
              <span
                className={`text-[11px] px-1.5 py-0.5 rounded-full font-bold ${
                  activeCategory === cat.id ? "bg-white/20 text-white" : "bg-gray-100/80 text-gray-500"
                }`}
              >
                {cat.count}
              </span>
        ...
        {products.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <button
              ...
              className="mt-6 px-5 py-2.5 bg-gray-900 text-white rounded-full text-sm font-semibold hover:bg-gray-800 transition-colors"
            >
              Clear Filters
            </button>
          </div>
