...
import { WishlistButtonLarge } from "../../../components/ui/WishlistButton/WishlistButton.jsx";
...
const ProductPage = () => {
  ...
  const normalizeProduct = (productData) => {
    const images = productData.product_images || productData.images || [];

    return {
      ...productData,
      images,
      variants: productData.variants || {
        storage: productData.storage_options || [],
        ram: productData.ram_options || [],
        colors: productData.colors || [],
      },
      specs: productData.specs || productData.specifications || null,
  ...
  const fetchProduct = async () => {
    try {
      ...
      if (foundProduct) {
        const normalizedProduct = normalizeProduct(foundProduct);

        setProduct(normalizedProduct);

        const defaults = {
          storage: normalizedProduct.variants?.storage?.[0] || "",
          ram: normalizedProduct.variants?.ram?.[0] || "",
          imageIndex: 0,
        };
  ...
  const handlePrevImage = () => {
    ...
    setSelectedOptions((prev) => ({
      ...prev,
      imageIndex:
        (prev.imageIndex - 1 + product.images.length) % product.images.length,
    }));
  };
  ...
  const activeImageIndex =
    product.images && product.images.length > selectedOptions.imageIndex
  ...
  const bubbleButtonClass =
    "bg-gradient-to-b from-gray-500 to-gray-800 shadow-[inset_0px_2px_4px_rgba(255,255,255,0.3),_0px_4px_8px_rgba(0,0,0,0.4)] ring-1 ring-gray-600 text-white transition-all hover:bg-gradient-to-b hover:from-gray-400 hover:to-gray-700 hover:scale-105 active:scale-95";
      ...
      <AnimatePresence>
        {isZoomed && (
          ...
          >
            <button
              onClick={() => setIsZoomed(false)}
              className="absolute top-6 right-6 w-12 h-12 rounded-full bg-white/20 flex items-center justify-center text-white hover:bg-white/40 border border-white/50 transition-colors"
            >
      ...
      <div className="max-w-[1500px] mx-auto px-6 flex flex-col lg:grid lg:grid-cols-12 gap-8 lg:gap-14 min-h-[750px] relative">
          ...
          <div className="relative w-full aspect-square md:aspect-[4/3] lg:aspect-auto lg:h-[600px] bg-gradient-to-br from-[#d9e8f5] via-[#e2ebf4] to-[#f4f7fa] border border-white/50 shadow-[0_8px_32px_rgba(0,0,0,0.03)] rounded-[3rem] p-6 flex flex-col items-center justify-center overflow-hidden">
            ...
            <div className="absolute top-6 right-6 z-20">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold ${bubbleButtonClass}`}
              >
        ...
        <div className="order-2 lg:order-1 lg:col-span-3 flex flex-col justify-start z-10 lg:pt-4 mt-8 lg:mt-0">
          ...
          <div className="bg-white/40 backdrop-blur-xl border border-white/60 rounded-3xl p-6 shadow-[0_8px_32px_rgba(0,0,0,0.04)] w-full max-w-sm flex flex-col mt-auto mb-6">
            <h3 className="font-semibold text-gray-800 mb-6 text-lg border-b border-gray-200/50 pb-2">
              ...
              {selectedOptions.storage && (
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium text-gray-900">
                    {extraCost > 0 ? `+ ₹${extraCost.toLocaleString()}` : "Included"}
                  </span>
                </div>
              )}
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`flex-1 py-4 rounded-full font-bold tracking-wide text-sm ${
              ...
              <div className="flex flex-wrap gap-2">
                {product.variants.storage.map((opt) => (
                  <button
                    ...
                    className={`flex items-center gap-2 py-2 px-4 rounded-full text-sm font-medium transition-all w-full md:w-auto flex-1 ${
                      selectedOptions.storage === opt
              ...
              <div className="flex flex-wrap gap-3">
                {product.variants.colors.map((color, idx) => (
                  <button
                    ...
                    className={`relative w-12 h-12 rounded-full outline-none transition-transform hover:scale-110 ${
                      selectedOptions.imageIndex === idx
                        ? "ring-2 ring-gray-900 ring-offset-2 ring-offset-[#e2ebf4]"
                        : ""
                    }`}
                    title={color.charAt(0).toUpperCase() + color.slice(1)}
                  >
                    <div className={`absolute inset-[2px] rounded-full shadow-inner ring-1 ring-black/10 ${getColorClass(color)}`}></div>
                  </button>
                ))}
              </div>
            </div>
                ...
                {product.variants.ram.map((opt) => (
                  <button
                    ...
                    className={`flex items-center gap-3 py-3 px-4 rounded-xl text-sm font-medium transition-all ${
                      selectedOptions.ram === opt
                        ? "bg-gray-100 border-gray-900 shadow-sm"
                        : "bg-white/50 border-transparent hover:bg-white"
                    } border`}
                  >
                    <div
                      className={`w-4 h-4 rounded-full border-[4px] ${
                        selectedOptions.ram === opt
                  ...
                  <tbody>
                    {Object.entries(product.specs).map(([key, val], idx) => (
                      <tr key={key} className={idx % 2 === 0 ? "bg-white/30" : ""}>
                        <td className="py-4 px-6 font-semibold text-gray-900 w-1/3 capitalize border-r border-white/50 bg-white/10">
                          {key}
                        </td>
