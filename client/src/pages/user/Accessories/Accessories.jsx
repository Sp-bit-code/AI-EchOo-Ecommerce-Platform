      ...
      <div className="flex items-end justify-between mb-6">
        ...
        {total > 1 && (
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => setIdx((index) => Math.max(0, index - 1))}
              disabled={idx === 0}
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
                idx === 0 ? disabledBtn : bubbleBtn
              }`}
            >
              <svg
            ...
            <button
              onClick={() => setIdx((index) => Math.min(total - 1, index + 1))}
              disabled={idx === total - 1}
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
                idx === total - 1 ? disabledBtn : bubbleBtn
              }`}
            >
              <svg
                className="w-4 h-4"
...
const AccessoriesPage = () => {
  ...
  const [appleIdx, setAppleIdx] = useState(0);
  ...
  const apple = all.filter((product) => isBrandMatch(product.brand, "Apple"));

  const cases = all.filter((product) =>
    product.name?.toLowerCase().includes("case")
  );

  const audio = all.filter((product) => {
