...
const getCls = (color) => colorMap[color?.toLowerCase()?.trim()] || "bg-gray-300";
...
const bubbleBtn =
  "bg-gradient-to-b from-gray-500 to-gray-800 shadow-[inset_0px_2px_4px_rgba(255,255,255,0.3),_0px_4px_8px_rgba(0,0,0,0.4)] ring-1 ring-gray-600 text-white hover:from-gray-400 hover:to-gray-700 hover:scale-105 active:scale-95";

const disabledBtn =
  "bg-white/30 border border-white/50 text-gray-300 cursor-not-allowed";
...
const ProductCard = ({ product }) => {
  ...
  const primaryImage =
    product.images?.[0]?.image_url || product.images?.[0] || "/no-image.png";

  const secondaryImage =
    product.images?.[1]?.image_url || product.images?.[1];
      ...
      >
        <div className="relative bg-gradient-to-br from-[#d9e8f5] via-[#e2ebf4] to-[#f4f7fa] aspect-square flex items-center justify-center overflow-hidden p-5">
          <motion.img
            src={primaryImage}
            alt={product.name}
            className={`absolute inset-0 h-full w-full object-contain mix-blend-multiply transition-all duration-500 ${
              showAltImage
          ...
          {secondaryImage && (
            <motion.img
              ...
              className={`absolute inset-0 h-full w-full object-contain mix-blend-multiply transition-all duration-500 ${
                showAltImage
                  ? "scale-[1.05] opacity-100 blur-0"
                  : "scale-[1.01] opacity-0 blur-[6px]"
              }`}
              onError={(event) => {
                event.target.style.display = "none";
              }}
            ...
            {product.variants?.colors?.length > 0 && (
              <div className="flex gap-1">
                {product.variants.colors.slice(0, 4).map((color, index) => (
                  <div
                    key={index}
                    className={`w-3 h-3 rounded-full ${getCls(color)} shadow-sm`}
                    title={color}
                  />
                ))}
      ...
      <div className="flex items-end justify-between mb-6">
        ...
        {total > 1 && (
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => setSectionIdx((index) => Math.max(0, index - 1))}
              disabled={sectionIdx === 0}
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
                sectionIdx === 0 ? disabledBtn : bubbleBtn
              }`}
            >
              <svg
            ...
            <button
              onClick={() =>
                setSectionIdx((index) => Math.min(total - 1, index + 1))
              }
              disabled={sectionIdx === total - 1}
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
                sectionIdx === total - 1 ? disabledBtn : bubbleBtn
              }`}
            >
              <svg
                className="w-4 h-4"
...
const AppleProductsPage = () => {
    ...
    const fetchAppleProducts = async () => {
      try {
        ...
        const list = Array.isArray(response)
          ? response
          : getProductsFromResponse(response?.data || response);

        if (ignore) return;

        setProducts(list.map(normalizeProduct));
      } catch (error) {
        console.error("Failed to load Apple products:", error);
  ...
  const iphones = products.filter(
    (product) =>
      isBrandMatch(product.brand, "Apple") &&
      isCategoryMatch(product.category, "smartphone")
  );

  const macs = products.filter(
    (product) =>
      isBrandMatch(product.brand, "Apple") &&
      isCategoryMatch(product.category, "laptop")
  );

  const accessories = products.filter(
    (product) =>
      isBrandMatch(product.brand, "Apple") &&
      isCategoryMatch(product.category, "accessory")
  );
    ...
    >
      ...
      <div className="max-w-[1500px] mx-auto px-4 sm:px-6">
        <Section
          ...
          emoji="Smartphones"
          products={iphones}
          sectionIdx={iphoneIdx}
          setSectionIdx={setIphoneIdx}
        />
        ...
        <Section
          ...
          products={macs}
          sectionIdx={macIdx}
          setSectionIdx={setMacIdx}
        />

        <Section
          title="Accessories"
          emoji="Accessories & More"
          products={accessories}
          sectionIdx={accIdx}
          setSectionIdx={setAccIdx}
        />
