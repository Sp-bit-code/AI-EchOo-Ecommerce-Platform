...
const PIE_COLORS_DATA = [
  { gradientId: "pieGradient1", color1: "#f97316", color2: "#ea580c" },
  { gradientId: "pieGradient2", color1: "#10b981", color2: "#059669" },
  { gradientId: "pieGradient3", color1: "#6366f1", color2: "#4f46e5" },
  { gradientId: "pieGradient4", color1: "#ef4444", color2: "#dc2626" },
  { gradientId: "pieGradient5", color1: "#3b82f6", color2: "#2563eb" },
];
...
const AnalyticsTooltip = ({ active, payload, label, formatCurrency }) => {
  if (active && payload && payload.length) {
    ...
    const ordersData = payload.find((item) => item.dataKey === "orders");
...
const PieTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    const total = payload.reduce((sum, entry) => {
      return sum + Number(entry.payload.value || 0);
    }, 0);
    ...
    return (
      <div className="bg-white/80 border border-white/60 shadow-xl rounded-2xl p-3 backdrop-blur-md font-inter">
        ...
        <p className="text-xs text-gray-500 mt-1">({percent}%) of sales</p>
      </div>
    );
  }
...
const AnalyticsCharts = ({
  ...
  const safeFormatCurrency = formatCurrency || defaultFormatCurrency;
  ...
  const totalRevenue = safeMonthlySales.reduce((sum, day) => {
    return sum + Number(day.revenue || 0);
  }, 0);

  const totalOrders = safeMonthlySales.reduce((sum, day) => {
    return sum + Number(day.orders || 0);
  }, 0);
  ...
  const peakRevenue =
    safeMonthlySales.length > 0
      ? Math.max(...safeMonthlySales.map((day) => Number(day.revenue || 0)))
      : 0;
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={safeMonthlySales}>
              ...
              <Tooltip
                content={
                  <AnalyticsTooltip formatCurrency={safeFormatCurrency} />
                }
                cursor={{
                  stroke: "#9ca3af",
                  strokeWidth: 1,
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <p className="text-lg font-bold text-gray-900 mt-1">
                {safeFormatCurrency(avgDailyRevenue)}
              </p>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <p className="text-lg font-bold text-gray-900 mt-1">
                {safeFormatCurrency(peakRevenue)}
              </p>
            </div>
          </div>
        </div>
            ...
            <PieChart>
              <defs>
                {safeCategoryData.map((entry, index) => {
                  const colorConfig =
                    PIE_COLORS_DATA[index % PIE_COLORS_DATA.length];
                  ...
                  return (
                    ...
                    >
                      <stop
                        offset="0%"
                        stopColor={colorConfig.color1}
                        stopOpacity={1}
                      />
                      <stop
                        offset="100%"
                        stopColor={colorConfig.color2}
                        stopOpacity={1}
                      />
                    </linearGradient>
                  );
            ...
            >
              <defs>
                ...
                >
                  <stop
                    offset="0%"
                    stopColor={BAR_COLORS.color1}
                    stopOpacity={1}
                  />
                  <stop
                    offset="100%"
                    stopColor={BAR_COLORS.color2}
                    stopOpacity={1}
                  />
                </linearGradient>
        ...
        <div className="space-y-4">
          {safeTopProducts.length > 0 ? (
            safeTopProducts.map((product, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-3 hover:bg-indigo-50/50 rounded-xl transition duration-200"
              >
                <div className="flex items-center">
